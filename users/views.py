...
def kakao_login(request):
    ...
    redirecdt_uri = "https://127.0.0.1:8000/users/login/kakao/callback"
    return redirect(
        f"https://kauth.kakao.com/oauth/authorize?client_id={client_id}&redirect_uri={redirecdt_uri}&response_type=code"
    )
...
def kakao_callback(request):
    try:
        ...
        token_request = requests.get(
            f"https://kauth.kakao.com/oauth/token?grant_type=authorization_code&client_id={client_id}&redirect_uri={redirecdt_uri}&code={code}"
        )
        token_json = token_request.json()
        error = token_json.get("error", None)
        if error is not None:
        ...
        nickname = properties.get("niknames")
